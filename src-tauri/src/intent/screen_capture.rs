    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            if CAPTURE_ENABLED.load(Ordering::Relaxed) {
                ...
                let result: Result<Result<(Option<String>, Option<Arc<RgbaImage>>), String>, _> =
                    tokio::task::spawn_blocking(move || capture_and_ocr_pipeline(count, prev_img))
                        .await;
                    ...
                    Ok(Ok((text, new_img))) => {
                        ...
                        if let Some(extracted_text) = text {
                            if !extracted_text.trim().is_empty() {
                                ...
                                let truncated =
                                    truncate_at_char_boundary(&extracted_text, MAX_OCR_CHARS);
...
fn capture_and_ocr_pipeline(
    count: u32,
    prev_image: Option<Arc<RgbaImage>>,
) -> Result<(Option<String>, Option<Arc<RgbaImage>>), String> {
    println!("\n[OCR] ── Capture #{} ──────────────────────", count);
    ...
    if let Some(ref info) = active_window_info {
        ...
        let target_window = windows
            .into_iter()
            .find(|w| w.title().contains(&info.title) || w.app_name().contains(&info.app_name));
    ...
    let screenshot: RgbaImage =
        RgbaImage::from_raw(screenshot_width, screenshot_height, screenshot_bytes)
    ...
    println!(
        "[OCR] ✅ OCR completed in {:.1}s (OCR part: {}ms). Found {} chars.",
        elapsed.as_secs_f64(),
...
fn run_ocr_with_variants(img: &RgbaImage) -> Result<String, String> {
    ...
    let variants: [(&str, std::borrow::Cow<RgbaImage>); 2] = [
        ...
        (
            "contrast",
            std::borrow::Cow::Owned(preprocess_for_text(img)),
        ),
    ];
    ...
    for (name, variant) in variants {
        ...
        variant
            .save(&temp_path)
            .map_err(|e| format!("Save temp {}: {}", name, e))?;
...
fn is_gibberish_line(line: &str) -> bool {
    ...
    let weird = chars
        ...
        .count() as f64
        / total;
    let vowels = chars.iter().filter(|c| "aeiouAEIOU".contains(**c)).count() as f64;
...
fn run_windows_ocr(image_path: &PathBuf) -> Result<String, String> {
    ...
    let engine =
        OcrEngine::TryCreateFromUserProfileLanguages().map_err(|e| format!("OcrEngine: {}", e))?;
